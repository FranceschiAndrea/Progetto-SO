...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){
  ...
  Resource_init();
  Descriptor_init();
  Semaphore_init();         //aggiunta di due funzioni per i semafori
  SemDescriptor_init();
  init_pcb=0;
  ...
  syscall_numarg[DSOS_CALL_SEMOPEN]      = 2;       //mi servono dueparametri per la open
  ...
  syscall_numarg[DSOS_CALL_SEMPOST]      = 1;//mi serve un paramtro per la semPost
...
//aggiungo funzione per la gestione della open tramite syscall
int disastrOS_semOpen(int n_semaforo,int sem_val){
